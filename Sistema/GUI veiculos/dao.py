...
class ClientDAO:
    ...
    def insert(self, veiculos):
        "insere novos registros no banco"
        try:
            self.bd.execute("INSERT INTO veiculos VALUES(NULL, ?,?,?,?,?,?,?,?,?,?,?,?,?)", (
                veiculo.marca, veiculo.modelo, veiculo.ano, veiculo.cor,veiculo.tanque, veiculo.combustivel, veiculo.consumo_cidade, veiculo.consumo_estrada, veiculo.tempo_0_100, veiculo.chassi, veiculo.placa, veiculo.tamanho_pneu,veiculo.som, veiculo.valor_diaria))
            self.bd.persist()
        except sqlite3.Error as error:
    ...
    def search(self, veiculo):
        ...
        rows = None
        try:
            self.bd.execute("SELECT * FROM clientes WHERE marca=? or modelo=? or ano=? or cor=?", (cliente.nome, cliente.modelo, cliente.ano, cliente.cor))
            rows = self.bd.fetchall()
        except sqlite3.Error as error:
...
class VeiculoDAO:
    ...
    def view(self):
        ...
        rows = None
        try:
            self.bd.execute("SELECT * FROM veiculo")    
            rows = self.bd.fetchall()
        except sql.Error as error:
            print("Falha ao tentar selecionar os registros")
            print("Classe da exceção: ", error.__class__)
    ...
    def insert(self, veiculo):
        "insere novos registros no banco"
        try:
            self.bd.execute("INSERT INTO veiculo VALUES(NULL, ?,?,?,?,?,?,?,?,?,?,?,?,?)", (veiculo.marca, veiculo.modelo, veiculo.ano, veiculo.cor,veiculo.tanque, veiculo.combustivel, veiculo.consumo_cidade, veiculo.consumo_estrada, veiculo.tempo_0_100, veiculo.chassi, veiculo.placa, veiculo.tamanho_pneu,veiculo.som, veiculo.valor_diaria))
            self.bd.persist()
        except sqlite3.Error as error:
            print("Falha ao tentar inserir os registros")
            print("Classe da exceção: ", error.__class__)
    ...
    def search(self, veiculo):
        ...
        rows = None
        try:
            self.bd.execute("SELECT * FROM veiculo WHERE marca=? or modelo=? or ano=? or cor=? or tanque =? or combustivel =? or consumo_cidade =? or consumo_estrada =? or tempo_0_100 =? or chassi =? or placa =? or tamanho_pneu =? or som =? or valor_diaria =?", (veiculo.marca, veiculo.modelo, veiculo.ano, veiculo.cor,veiculo.tanque, veiculo.combustivel, veiculo.consumo_cidade, veiculo.consumo_estrada, veiculo.tempo_0_100, veiculo.chassi, veiculo.placa, veiculo.tamanho_pneu,veiculo.som, veiculo.valor_diaria))
            rows = self.bd.fetchall()
        except sqlite3.Error as error:
            print("Falha ao tentar buscar os registros")
            print("Classe da exceção: ", error.__class__)
    ...
    def update(self, id, veiculo):
        "atualiza registros no banco"
        try:
            self.bd.execute("UPDATE veiculo SET marca =?, modelo=?, ano=?, cor=?, tanque =?, combustivel =?, consumo_cidade =?, consumo_estrada =?, tempo_0_100 =?, chassi =?, placa =?, tamanho_pneu =?, som =?, valor_diaria =? WHERE id = ?",(veiculo.marca, veiculo.modelo, veiculo.ano, veiculo.cor,veiculo.tanque, veiculo.combustivel, veiculo.consumo_cidade, veiculo.consumo_estrada, veiculo.tempo_0_100, veiculo.chassi, veiculo.placa, veiculo.tamanho_pneu,veiculo.som, veiculo.valor_diaria, id))
            self.bd.persist()
        except sqlite3.Error as error:
            print("Falha ao tentar atualizar os registros")
            print("Classe da exceção: ", error.__class__)
    ...
    def delete(self, id):
        "remove registros no banco"
        try:
            self.bd.execute("DELETE FROM veiculo WHERE id = ?", (id,))
            self.bd.persist()
        except sqlite3.Error as error:
            print("Falha ao tentar remover o registro")
            print("Classe da exceção: ", error.__class__)
